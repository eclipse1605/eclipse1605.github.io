...
After the meeting, I went back to reading about Wasserstein Gradient Flows and trying to build a cleaner mental picture of why they are such a good fit here. The main appeal is that they turn inference into dynamics on distributions: instead of updating a single point estimate, the method updates a whole particle system in a geometry shaped by optimal transport. That is a nice match for a method like PrO, where the target object is already a predictive distribution and the implementation naturally lives in the space of measures.
